...
#include "Simulator.h"
#include "SimulatorMachineFunctionInfo.h"
...
static const MCPhysReg ArgGPRs[] = {Simulator::R9, Simulator::R10, Simulator::R11, Simulator::R12};

void SimulatorTargetLowering::ReplaceNodeResults(SDNode *N,
...
#include "SimulatorGenCallingConv.inc"
...
SDValue SimulatorTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_Simulator);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, Simulator::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(SimulatorISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_Simulator);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const SimulatorSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const SimulatorTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
SDValue SimulatorTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Simulator);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &Simulator::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    SimulatorFunctionInfo *UFI = MF.getInfo<SimulatorFunctionInfo>();
...
bool SimulatorTargetLowering::CanLowerReturn(
...
SDValue
SimulatorTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Simulator);
  ...
  return DAG.getNode(SimulatorISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue SimulatorTargetLowering::PerformDAGCombine(SDNode *N,
...
bool SimulatorTargetLowering::isLegalAddressingMode(const DataLayout &DL,
